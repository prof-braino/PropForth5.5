...
\
\ setDriftCorrection ( n1 -- ) sets the time correction to n1 microseconds per day
\
...
lockdict variable _driftCorrectionNumerator 4 allot freedict 1 u>d _driftCorrectionNumerator dL!
]
[ifndef _driftCorrectionDenominator
lockdict variable _driftCorrectionDenominator 4 allot freedict 1 u>d _driftCorrectionDenominator dL!
]
\
...
: _dtickCNT@
	_dtickCNT _timelock __tickCNTat  
;
...
\ getTime ( -- y m d h m s ticks -- ) gets UTC time
: getTime
	_dgettime _driftCorrectionNumerator dL@ _driftCorrectionDenominator dL@ du*/ _t>ymdhms
;
\ getLocalTime ( -- y m d h m s ticks -- ) gets local time
: getLocalTime
	_dgetLocaltime  _driftCorrectionNumerator dL@ _driftCorrectionDenominator dL@ du*/ _t>ymdhms
;
\
\ formatTime ( -- y m d h m s ticks -- cstr) formats a to a printable sortable string
: formatTime
...
: setDriftCorrection
		...
		d_1_000_000 d_86_400 um* ddup _driftCorrectionDenominator dL!
		
		d+ _driftCorrectionNumerator dL!
;
\
\ utc ( -- ) print the utc time
: utc 
