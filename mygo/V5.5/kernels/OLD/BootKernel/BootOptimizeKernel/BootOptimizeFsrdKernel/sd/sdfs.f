...
\
\
\ uncomment this line to include the buffered version of fwrite
\ fbwrite
\ 
\
\ 1 wconstant build_sdfs_fbwrite
\
\
\
...
;
\
\ sd_appendblk( addr size headerblk -- )
: sd_appendblk
	dup sd_blockread
\							( addr size fileblock -- )
...
\
\
c" onboot" find drop pfa>nfa 1+ c" onbold" C@++ rot swap cmove
\
: onboot
	onbold 1 sd_mount 
\ if esc has been hit do not execute autoload.f
	h6 cogio W@ h1B <>
	if
		c" .autoload.f" sd_load
	then
;
}
\
\
[ifdef build_sdfs_fbwrite
\
\ iobuf buffers up to 128 characters in an io channel, very useful when you want to run a series
\ of synchronous commands asynchronously
\
\
\ t0 - the output pointer
\ t1 - the input pointer
\ pad - the buffer
\
\ _iobufout ( -- ) if there is a character in the buffer, and the output is ready, emit
: _iobufout
	t0 W@ t1 W@ <>
	if
		t0 W@ pad + C@ femit?
...
\ we have a key
				dup hFF =
				if
\ end sequence FF FF FF, flush the output buffer, then exit
					2drop -1
					begin
						_iobufout
						t0 W@ t1 W@ =
...
\ we have a key, buffer it
					t1 W@ pad + C! t1 W@ 1+ h7F and t1 W!
				then
			else
				drop
			then
...
\
h4 wconstant _fb_nb
h204 _fb_nb u* wconstant _fb_sz
\
\
\ _fb1 ( addr keys -- addr keys )
: _fb1
	over h4 + h200 bounds
	do
\				check to see if we have a ... at the end of a line
...
;
\
\ _fbR1 ( filename numblocks_to_allocate currentdir_block -- headerblock currentblock bytes_written)
: _fbR1
	sd_init v_currentdir COG! sd_createfile dup 1+ 0
;
\
\ _fbR2 ( headerblock currentblock bytes_written addr -- headerblock currentblock bytes_written)
: _fbR2
	dup W@ over 4+ sd_cogbuf rot h4 u/mod swap if 1+ then mem>cog
	rot dup sd_blockwrite 1+
	rot2 dup W@ 0 rot W!
	+ 
;
\
\
\ _fbR3 ( headerblock currentblock bytes_written --)
: _fbR3
	rot dup sd_blockread swap sd_cogbuf h2A + COG! sd_blockwrite drop
;
\
\
...
\ ( num_blocks_to_allocate cstr -- )
			dictend W@ dup >r
			_fb_sz 4+ - h3 andn dup dictend W!
 			freedict
\ ( num_blocks_to_allocate cstr addr -- ) ( olddictend -- )

			cogid nfcog dup dup >r dup >r cogstate 2 swap orC! iolink ." iobuf~h0D"
\
\ ( num_blocks_to_allocate cstr addr -- ) ( olddictend nfcog nfcog -- )
\
			r> nfcog
			dup cogstate h2 swap orC!
			iolink
			swap ." c~h22 " .cstr h22 emit space 
			over . v_currentdir COG@ .
			." _fbR1~h0D"
\ ( num_blocks_to_allocate addr -- )
			key _fsk _fsk _fsk
\ ( num_blocks_to_allocate addr keys -- )
			over 0 over W!
			0 swap h204 + W!
			rot 1- _fb_nb u/  0
			do
				...
				then
			loop
			." _fbR3~h0D~h0D~h0D~hFF~hFF~hFF"

\			r> 2 over cogstate andnC! iounlink
			cogid iounlink
\
			2drop
			r> dictend W!
...
[ifdef _sd_debug
\
\ sd_lsdbg ( -- ) list the current directory
: sd_lsdbg
	v_currentdir COG@ 1+ h80 bounds
	do
		i sd_blockread
		." BLOCK " i .
		-1 sd_cogbuf h80 bounds
		do
			h20202000 i COG@ = and
			i 1+ h6 bounds
			do
...
\
\ _sdblck ( flag offset -- )
: _sdblck sd_cogbuf + COG@ 20202020 = and ;
\
: sd_dbg
	v_currentdir COG@ sd_blockread
	sd_cogbuf h2E + COG@ dup v_currentdir COG! sd_blockread
	sd_cogbuf h36 + COG@ v_currentdir COG@
	do
		i sd_blockread
		." ~h0D  Block#: " i . 
		." ~h0DPathname: " sd_cogbuf cog>pad pad .cstr
		." ~h0DFilename: " sd_cogbuf h20 + cog>tbuf7 tbuf .cstr  
		." ~h0D  Length: " sd_cogbuf h2A + COG@ .
		." ~h0D #Blocks: " sd_cogbuf h2B + COG@ .
		." ~h0D Rootdir: " sd_cogbuf h2E + COG@ .
		." ~h0D     Dir: " sd_cogbuf h2F + COG@ .
		." ~h0DFirstBlk: " sd_cogbuf h32 + COG@ .
		." ~h0D LastBlk: " sd_cogbuf h33 + COG@ .
		i v_currentdir COG@ =
		if
			." ~h0D FreeBlk: " sd_cogbuf h36 + COG@ .
		then
		-1 
		h27 _sdblck
		h28 _sdblck
...
\
		...
		if
			...
			v_currentdir COG@
			sd_cogbuf h2F + COG@ v_currentdir COG!
			sd_cogbuf h20 + cog>pad pad sd_find
			i =
			if
				." ~h09~h09GOOD"
			else
...
\
: lm
	c" LOG: cnt: " pad ccopy cnt COG@ _lf pad cappend
	pad C@
	c"  This message will be truncated to different lengths every time." pad cappend
	h40 rnd h100 u*/ + pad C!
	c" ~h0D" pad cappend pad
	C@++ c" testlog" sd_append drop padbl
;
\
: mlm
	h1000 0 do lm key? if key h1B = if leave then then loop
;
\
: lsd sd_lsdbg ;
]
