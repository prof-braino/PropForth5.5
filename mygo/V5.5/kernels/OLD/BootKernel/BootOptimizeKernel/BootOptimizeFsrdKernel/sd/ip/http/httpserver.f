...
: ip_SOCKhttp
	...
	if
		...
		httpport over ip_SOCKlistenport W!		\ set the port
...
\ (cstr1 headerblock -- )
	...
	if
		...
		if
			...
			dup sd_blockread
			base W@ sd_cogbuf h2A + COG@ decimal . ." ~h0D~h0A~h0D~h0A"
			base W!
			sd_readblk
		else
			2drop httpabort
