fl
{
GY-80 Simple with LOGGING no Graph.
...
h3 fwrite ADXL345-driver.f

{
free 9068 bytes free - 44 cog longs free - before loading ADXL345-driver
free 8722 bytes free - 44 cog longs free - after loading ADXL345-driver
delta 346
}
...
\ ADXL.initialize ADXLrXYZ! ADXL.XYZDATA W@ .
\ ADXLrXYZ! ADXL.XYZDATA W@ .

\ nn fwrite ADXL345-driver
...

\   fl

h4 fwrite HMC5883-driver.f
{
free 8722 bytes free - 44 cog longs free - before HMC5883-driver
free 8142 bytes free - 44 cog longs free- after HMC5883-driver
delta 580
}
...
\ nn fwrite HMC5883-driver

...

\  fl    \ testing DRIVER20140607

h7 fwrite L3G4-driver.f
{
free 8142 bytes free - 44 cog longs free - before L3G4-driver 
free 7164 bytes free - 44 cog longs free - after L3G4-driver
delta 978 (big - can be factored)
}
...
\             0 _eeread 0 _eeread 8 lshift or \ z
             0 _eeread -1 _eeread 8 lshift or \ z
...
\     0 _eeread 0 _eeread 8 lshift or L3G4200D.DATA 6 + W! \ z
     0 _eeread -1 _eeread 8 lshift or L3G4200D.DATA 6 + W! \ z
...
: L3G.data!
             ...
             _L3Gr 
     0 _eeread                       L3G4200D.DATA     C! \ temperature 
    0 _eeread   drop \             L3G4200D.DATA 1+  C! \ status    
     0 _eeread 0 _eeread 8 lshift or L3G4200D.DATA 2+  W! \ x
     0 _eeread 0 _eeread 8 lshift or L3G4200D.DATA 4+  W! \ y
    0 _eeread 0 _eeread 2drop \ L3G4200D.DATA 6 + W! \ z
     0 _eeread -1 _eeread 8 lshift or L3G4200D.DATA 6 + W! \ z
...
: L3G.initialize
  ...
  b0000_1111  h20 !L3G8  \ min speed, power on
\  b0010_0111  h21 !L3G8  \ normal 200Hz  0.1
  b0000_0000  h22 !L3G8  \ idsable interupts (default)
  b0000_0000  h23 !L3G8  \ continuous, LSB lowest, CHECK FULL SCALE
  b0000_0000  h24 !L3G8  \ boot, no FIFO, no HiPass, def INT 
;


\ nn fwrite L3G4-driver
{
[
[ifdef  build_bmp085
        : AveTemp
...
h8 fwrite BMP085-driver.f

{
free 7164 bytes free - 44 cog longs free - before BMP085-driver
free 6184 bytes free - 44 cog longs free - after BMP085-driver
delta 980
}
...
\ nn fwrite BMP085-driver
...


\  fl

h7 fwrite BMP085-N-display.f

1 wconstant build_bmp085-N
...
h7 fwrite BMP085altimeter-support.f

{
free 6184 bytes free - 44 cog longs free - before BMP085altimeter-support
free 4982 bytes free - 44 cog longs free - after BMP085altimeter-support
delta 1202 (biggest)
}
...
\ nn fwrite BMP085altimeter-support

...

\ CRAPS out on load HERE 20140529 - ok 20140531

\  fl

h2 fwrite GY80-Background.f

{
free 4982 bytes free - 44 cog longs free - before GY80-Background
free 4858 bytes free - 44 cog longs free - after GY80-Background
delta 124
}

1 wconstant build_GY80-BG

: (signed)  dup h8000 and if hFFFF0000 or then  ;


[ifdef build_L3G4200D
            L3G.data! \ L3G4200D  
]
...
: do.GY80
        4 state andnC!
        c" GY80-BG" cds W! \ text displayed by cog?
...
free 4858 bytes free - 44 cog longs free - before ANSI
free 4666 bytes free - 44 cog longs free - after ANSI
delat 210
}
1 wconstant build_ANSI
...
: AT csi .digits h3B emit .digits h66 emit  ;
: home       csi ." 1;1f" ;
: clear      csi ." 2J" ; 
: cls home clear  ;
...
h1 fwrite ADXL345-N-display.f

{
free 4648 bytes free - 44 cog longs free - before ADXL345-N-display.f
free 4427 bytes free - 44 cog longs free - after ADXL345-N-display.f
delta 176
}
1 wconstant build_ADXL345-N-display
...
{
: .ADXL
      ." ADXL.X=" @ADXL.X . 3 spaces
      ." ADXL.Y=" @ADXL.Y . 3 spaces
      ." ADXL.Z=" @ADXL.Z . 3 spaces
;
...
: .ADXL
          ." ADXL "
            @ADXL (signed) . (signed) . (signed)  .  3 spaces
;

\  nn fwrite ADXL345-N-display.f
...
h4 fwrite HMC5883-N-display.f

{
free 4472 bytes free - 44 cog longs free - before HMC5883-N-display.f
free 4070 bytes free - 44 cog longs free - after HMC5883-N-display.f
delta 402
}

1 wconstant build_HMC5883-N-display
...
\ @mag (- z y x ) 
: @mag
        HMC5883L.XYZDATA 2+ W@  \ z (signed)
        HMC5883L.XYZDATA 4+ W@ \ Y (signed) ;
        HMC5883L.XYZDATA    W@ \ x (signed) ;
;
...
: .HMC
      ." HMC "
       @mag (signed) . (signed) . (signed) . 3 spaces
;

\ end HMC5883-N-display.f

...

\  fl

h2 fwrite L3G4-N-display.f

1 constant build_L3G4200D-N

{
free 4070 bytes free - 44 cog longs free - before L3G4-N-display.f
free 3844 bytes free - 44 cog longs free - after L3G4-N-display.f
delta 226
}
...
h8 fwrite Graph-display.f

1 constant build_Graph
...
\ fl
 ...
 10 fwrite ADXL345-G-display.f

\  ADXL345 Aceelerometer graphic

1 wconstant build_ADXL345_G
...
\ nn fwrite ADXL345 Graph-display
...
h4 fwrite HMC5883-G-display.f

1 wconstant build_HMC5883-G
...
: .magY 
HMC5883L.XYZDATA 4+ W@  (signed)  \ Y (signed) 
...
: .magZ 
HMC5883L.XYZDATA 2+ W@ (signed)  \ z (signed)
...
: .magX 
HMC5883L.XYZDATA    W@ (signed)  \ x (signed) 
...
h2 fwrite L3G4-G-display.f

1 wconstant build_L3G4200D-G
...
\   fl  
h5 fwrite GY80-main-loop

: GY-80

\ 5 cogreset 100 delms 
\ 4 cogreset 100 delms 
sc 
...
[ifdef build_ADXL345_G 
.accY  .accZ  .accX 
]
[ifdef build_HMC5883-G
.magY  .magZ .magX
]
[ifdef build_L3G4200D-G
.pitch .yaw .roll
]
[ifdef build_Graph 
@centerColumn 5 +  @centerRow 3 + AT 
]
[ifdef build_L3G4200D-N
.L3G4200D
]
cr 
[ifdef build_Graph
@centerColumn 5 +  @centerRow 4 + AT 
]

[ifdef build_bmp085-N \ .BMP085   \ build_bmp085
.BMP085 2 spaces
]
[ifdef build_bmp085alt
." Altitude " .alt 5 spaces
]
cr
[ifdef build_Graph 
@centerColumn 5 +  @centerRow 5 + AT 
]
...
cr
[ifdef build_Graph 
@centerColumn 5 +  @centerRow 7 + AT 
]
[ifdef AveTemp
." Average Temp " AveTemp .
] 
